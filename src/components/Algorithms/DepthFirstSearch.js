export const DepthFirstSearch = (grid,start,finish)=>{
...
const getNeighbours = (node,grid)=>{
    ...
    if(currRow < row-1){
        if(!grid[currRow+1][currCol].isWallAnimate){
            neighbours.unshift(grid[currRow+1][currCol])
        }
    }
    if(currCol < col-1){
        if(!grid[currRow][currCol+1].isWallAnimate){
            neighbours.unshift(grid[currRow][currCol+1])
        }
    }
    if(currRow > 0){
        if(!grid[currRow-1][currCol].isWallAnimate ){
