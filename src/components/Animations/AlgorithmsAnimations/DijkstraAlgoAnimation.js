const DijkstraAlgoAnimation = (animations,path)=>{
    ...
    for(let i = 0; i <= length; i++){
        ...
        setTimeout(()=>{
            ...
            const id = `${row}+${col}`
            document.getElementById(id).className = isStart?"node node_start":isFinish?"node node_finish":"node node_visited_shortestPath"
        },i*13)
    }
...
const shortestPathAnimation = (animations)=>{
    ...
    for(let i = 0; i < length; i++){
        setTimeout(()=>{
            const {row,col,isStart,isFinish} = animations[i]
            const id = `${row}+${col}`
            document.getElementById(id).className = isStart?"node node_start":isFinish?"node node_finish":"node node_shortestPath"
        },i*25)
    }
 }
...
export default DijkstraAlgoAnimation
